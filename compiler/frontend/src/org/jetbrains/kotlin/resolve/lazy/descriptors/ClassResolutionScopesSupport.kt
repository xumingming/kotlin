    ...
    private val inheritanceScope: () -> LexicalScope = storageManager.createLazyValueWithPostCompute(
            {
                ...
                var parent: ClassDescriptor? = classDescriptor
                do {
                    superClassesAndMe.add(parent)
                    parent = parent!!.getSuperClassNotAny()
                }
                while(parent != null && parent != classDescriptor)

                superClassesAndMe.asReversed().fold(getOuterScope()) { scope, currentClass ->
                    createInheritanceScope(scope, ownerDescriptor = classDescriptor, classDescriptor = currentClass)
                }
            },
