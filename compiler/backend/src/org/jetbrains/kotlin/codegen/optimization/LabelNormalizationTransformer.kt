...
public class LabelNormalizationTransformer : MethodTransformer() {
    ...
    private fun rewriteInsns(methodNode: MethodNode) {
        var thisNode = methodNode.instructions.getFirst()
        while (thisNode != null) {
            thisNode = when (thisNode.getType()) {
                AbstractInsnNode.JUMP_INSN ->
                    rewriteJumpInsn(methodNode, thisNode as JumpInsnNode)
                AbstractInsnNode.LINE ->
                    rewriteLineNumberNode(methodNode, thisNode as LineNumberNode)
                AbstractInsnNode.LOOKUPSWITCH_INSN ->
                    rewriteLookupSwitchInsn(methodNode, thisNode as LookupSwitchInsnNode)
                AbstractInsnNode.TABLESWITCH_INSN ->
                    rewriteTableSwitchInsn(methodNode, thisNode as TableSwitchInsnNode)
                AbstractInsnNode.FRAME ->
                    rewriteFrameNode(methodNode, thisNode as FrameNode)
                else ->
                    thisNode.getNext()
            }
        }
