...
internal fun insertTryCatchBlocksMarkers(methodNode: MethodNode) {
    ...
    val defaultHandlerStartLabels = linkedSetOf<LabelNode>()
    for (tcb in methodNode.tryCatchBlocks) {
        if (tcb.isDefaultHandlerNode()) {
            defaultHandlerStartLabels.add(tcb.start)
        }
    }

    val decompiledTryDescriptorForStart = hashMapOf<LabelNode, DecompiledTryDescriptor>()
    val decompiledTryDescriptorForHandler = linkedMapOf<LabelNode, DecompiledTryDescriptor>()
