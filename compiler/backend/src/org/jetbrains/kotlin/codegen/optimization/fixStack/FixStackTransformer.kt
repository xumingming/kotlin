...
public class FixStackTransformer : MethodTransformer() {
    ...
    private fun transformAfterInlineCallMarker(
        ...
        if (afterInlineFrame != null && savedStackDescriptor.isNotEmpty()) {
            if (afterInlineFrame.getStackSize() > 0) {
                ...
                val valuesToPop = afterInlineStackValues.dropLast(1).reverse()
                actions.add({
                                restoreStackWithReturnValue(methodNode, inlineMarker, savedStackDescriptor,
                                                            returnValue, returnValueLocalVarIndex, valuesToPop)
                            })
            }
            else {
    ...
    private fun transformBeforeInlineCallMarker(
        ...
        if (savedStackValues != null) {
            ...
            val savedStackDescriptor = localVariablesManager.allocateVariablesForBeforeInlineMarker(inlineMarker, emptyList())
            actions.add({ methodNode.instructions.remove(inlineMarker) })
        }
    }
