...
//        generateDelegationsToPreviouslyCompiled(generateCallableMemberTasks)
    ...
    public fun generateClassOrObject(classOrObject: JetClassOrObject) {
        ...
        val packagePartType = state.fileClassesManager.getFileClassType(file)
    ...
    private fun generatePart(
        ...
        val partClassInfo = state.fileClassesManager.getFileClassInfo(file)
        val partType = AsmUtil.asmTypeByFqNameWithoutInnerClasses(partClassInfo.fileClassFqName)
        val partContext = CodegenContext.STATIC.intoMultifileFacadePart(packageFragment, facadeClassType, partType)
...
//        packageParts.parts.add(name.substring(name.lastIndexOf('/') + 1))
        ...
        val facadeContext = CodegenContext.STATIC.intoMultifileFacade(packageFragment, facadeClassType, partType)
