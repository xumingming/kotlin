...
public class MultifileClassCodegen(
    ...
    private fun generateDelegatesToPreviouslyCompiledParts(
        ...
        for (callables in previouslyCompiledCallables) {
            val partFqName = JvmFileClassUtil.getPartFqNameForDeserializedCallable(callables)
            val partType = AsmUtil.asmTypeByFqNameWithoutInnerClasses(partFqName)

            generateCallableMemberTasks[callables] = { generateDelegateToCompiledMember(callables, compiledPackageFragment, partType) }
        }
    }

    private fun generateDelegateToCompiledMember(
            member: DeclarationDescriptor,
        ...
        if (member is DeserializedSimpleFunctionDescriptor) {
        ...
        else if (member is DeserializedPropertyDescriptor) {
    ...
    companion object {
        ...
        private fun getCompiledPackageFragment(facadeFqName: FqName, state: GenerationState):
            ...
            if (!IncrementalCompilation.ENABLED) return null
